...
/**
 * GET - Retrieve master_category data
 * Endpoint to fetch all master_category data
 */
export const GET = async req => {
  try {
    // Parse request parameters
    const params = await checkPayload('get', req);
    const { data } = await getData('master_category', params);
    console.log(data);
    if (data) {
      return new Response(
        JSON.stringify({
      ...
      return new Response(
        JSON.stringify({
          success: false,
          message: 'No master_category data found',
        }),
        { status: 404 },
      );
    }
  } catch (error) {
    console.error('❌ Error fetching master_category data:', error);
    return new Response(
      JSON.stringify({
        success: false,
...
/**
 * POST - Create new master_category data
 * Endpoint to create a new master_category record
 */
export const POST = async req => {
  try {
    // Parse request body
    const params = await checkPayload('post', req);
    const data = await insertData('master_category', params);
    console.log(data);
    if (data.status) {
      return new Response(
        JSON.stringify({
...
/**
 * PUT - Update existing master_category data
 * Endpoint to update a master_category record by ID
 */
export const PUT = async req => {
  try {
    // Parse request body
    const { body, where } = await checkPayload('put', req);
    const data = await updateData('master_category', body, where);
    console.log(data);
...
/**
 * DELETE - Remove master_category data
 * Endpoint to delete a master_category record by ID
 */
export const DELETE = async req => {
  try {
    const params = await checkPayload('delete', req);
    const data = await deleteData('master_category', params);
    if (data.status) {
      return new Response(
        JSON.stringify({
          success: true,
